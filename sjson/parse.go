...
func ParseJSONFromString(s string, partialKey ...string) (*Value, error) {
	// find partialKey
...
func ParseJSONFromStringBytes(b []byte, partialKey ...string) (*Value, error) {
	return ParseJSONFromString(b2s(b), partialKey...)
}
...
func parseValue(s string, c *cache, depth int) (*Value, string, error) {
	...
	if s[0] == '{' {
		v, tail, err := parseJSONObject(s[1:], c, depth)
		if err != nil {
	...
	if s[0] == '[' {
		v, tail, err := parseJSONArray(s[1:], c, depth)
		if err != nil {
...
func parseJSONArray(s string, c *cache, depth int) (*Value, string, error) {
	s = skipWS(s)
	if len(s) == 0 {
...
func parseJSONObject(s string, c *cache, depth int) (*Value, string, error) {
	s = skipWS(s)
	if len(s) == 0 {
		return nil, s, fmt.Errorf("missing '}'")
	}
